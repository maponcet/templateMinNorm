...
%     srcERP(:,46:90) = createSourceERP(numROIs,ac_sources(1:length(ac_sources)/2),ac_sources((length(ac_sources)/2+1):end));
    ...
    srcExtERP(:,46:90) = createSourceERP(length(sourceExt),1); % create 1 ERP, repeated for the 2nd source (bilateral)
...
%             figure;plot(roiNormVis);hold on; plot(roiNorExt)
            ...
            fwdMatrixN1 = fwdMatrix; fwdMatrixN2 = fwdMatrix;
            fwdMatrixN1(:,idxExtROIfwd{iSub,1}) = scalingFactor * fwdMatrix(:,idxExtROIfwd{iSub,1}); 
            normFwd{iSub} = fwdMatrixN1;
...
save('simulOutput/simulTestNormExt1src.mat','simulERP','-v7.3')
...
%%% plot metrics
modName = {'ERP no interference','ERP interference','ERP 50interference'};
figure;hold on
for model=1:nbModel
...
saveas(gcf,['figures' filesep 'simulTestNormExt1src'],'png')
