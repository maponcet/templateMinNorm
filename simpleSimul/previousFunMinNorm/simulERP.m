...
[betaAverageModNorm, lambdaModNorm] = minNormFast_lcurve(stackAvMapNorm, stackY);
...
% [aucWholeLC(repBoot,totSbj,level), energyWholeLC(repBoot,totSbj,level),...
%     mseWholeNormLC(repBoot,totSbj,level)] = computeMetrics(retrieveWholeLC(:,winERP),srcERP(:,winERP));
% [aucROILC(repBoot,totSbj,level), energyROILC(repBoot,totSbj,level),...
%     mseROINormLC(repBoot,totSbj,level)] = computeMetrics(retrieveROILC(:,winERP),srcERP(:,winERP));
