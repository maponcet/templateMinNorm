...
listSub = repmat(randperm(50,10),3,1);
...
for boot=1:5
    ...
    for level=1:length(SNRlevel)
        ...
        %     test = reshape(stackY,[128,30,180]);
        %     Y_avg = permute(test,[2,1,3]);
        ...
        [betaBest_LcurveAvg, beta_LcurveAvg, lamda_LcurveAvg, ...
            lambdaGrid_LcurveAvg,lambdaCurvAvg] = minNorm_lcurve(  avMap, squeeze(mean(Y_avg)));
        clear mseTruth
        for iLambda = 1:length(beta_Lcurve)
            mseTruth(iLambda) = sum(sum((beta_Lcurve{iLambda}-srcERP).^2));
            [aucAve(iLambda), energyAve(iLambda), mseAve(iLambda)] = computeMetrics(beta_Lcurve{iLambda}(:,winERP),srcERP(:,winERP));
        end
        figure;
        ...
        semilogy([lambdaCurvAvg lambdaCurvAvg],ylim,'k--','linewidth',2)
        xlabel('lambda');ylabel('log MSE')
        legend('','lcfun','lcorner','avg')
%         saveas(gcf,['figures' filesep 'bootRep' num2str(level) '_SNR' num2str(noiseLevel)],'png')
        
        figure;
        subplot(1,3,1)
        semilogx(lambdaGrid_Lcurve,aucAve,'lineWidth',2);hold on;
        plot([lamda_Lcurve lamda_Lcurve],ylim,'g:','linewidth',2)
        plot([lambdaCurv lambdaCurv],ylim,'r:','linewidth',2)
        plot([lambdaCurvAvg lambdaCurvAvg],ylim,'k:','linewidth',2)
        xlabel('lambda');ylabel('AUC')
        subplot(1,3,2);semilogx(lambdaGrid_Lcurve,energyAve,'lineWidth',2);hold on;
        plot([lamda_Lcurve lamda_Lcurve],ylim,'g:','linewidth',2)
        plot([lambdaCurv lambdaCurv],ylim,'r:','linewidth',2)
        plot([lambdaCurvAvg lambdaCurvAvg],ylim,'k:','linewidth',2)
        xlabel('lambda');ylabel('energy')
        subplot(1,3,3);semilogx(lambdaGrid_Lcurve,mseAve,'lineWidth',2);hold on;
        plot([lamda_Lcurve lamda_Lcurve],ylim,'g:','linewidth',2)
        plot([lambdaCurv lambdaCurv],ylim,'r:','linewidth',2)
        plot([lambdaCurvAvg lambdaCurvAvg],ylim,'k:','linewidth',2)
        xlabel('lambda');ylabel('MSE')
%         saveas(gcf,['figures' filesep 'bootRepMetrics' num2str(level) '_SNR' num2str(noiseLevel)],'png')
        
    end
end

save('newData','stackAvMap', 'stackY')
