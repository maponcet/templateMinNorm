...
% always N=50
% compare with/out source for template, whole, ROI
...
%                 Y_avgExt(iSub,:,:) = bsxfun(@minus,squeeze(Yext(iSub,:,:)), mean(squeeze(Yext(iSub,:,:))));
            ...
            regionWholeExt50 = regionWhole;
            regionROIExt50 = regionWhole;
...
%             indFwd_noise=fullFwd{iSub};
            ...
            for iSub=1:numSubs
                ...
                [betaWholeExt50,lambdaWholeExt50] = minNormFast(fullFwd{iSub}, squeeze(Y_avgExt50(iSub,:,:)), nLambdaRidge);
%                 [betaWholeLC, lambdaWholeLC] = minNormFast_lcurve(fullFwd{iSub}, squeeze(Y_avg(iSub,:,:)));
                ...
                [betaROIExt50, lambdaROIExt50] = minNormFast([roiFwd{iSub,:}], squeeze(Y_avgExt50(iSub,:,:)), nLambdaRidge);
%                 [betaROILC, lambdaROILC] = minNormFast_lcurve([roiFwd{iSub,:}], squeeze(Y_avg(iSub,:,:)));
                ...
                regionROIExt50(iSub,:,:) = cell2mat(arrayfun(@(x) sum(betaROIExt50(range(x)+1:range(x+1), :)),1:numROIs,'uni',false)');
%                 regionROILC(iSub,:,:) = cell2mat(arrayfun(@(x) sum(betaROILC(range(x)+1:range(x+1), :)),1:numROIs,'uni',false)');
                ...
                regionWholeExt50(iSub,:,:) = cell2mat(arrayfun(@(x) sum(betaWholeExt50(idxROIfwd{iSub,x},:)),1:numROIs,'uni',false)');
...
%                 [betaROIinLC(iSub,:,:), lambdaGridMinNormROIinLC] = minNormFast_lcurve(sbjROI, squeeze(Y_avg(iSub,:,:)));
            ...
            retrieveWholeExt50 = squeeze(mean(regionWholeExt50,1));
            retrieveROIExt50 = squeeze(mean(regionROIExt50,1));
...
%             simulERP(repBoot,totSbj,level).beta(2,:,:) = betaAverageExt;
            ...
            simulERP(level).beta(4,:,:) = retrieveWholeExt50;
            simulERP(level).beta(5,:,:) = retrieveROI;
            simulERP(level).beta(6,:,:) = retrieveROIExt50;
...
%     end % sbj
    ...
    save(['simulOutput/interference/simulV1MTinterferenceERP_Comp' num2str(repBoot) '.mat'],'simulERP')
