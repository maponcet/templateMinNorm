clearvars;close all;
% random sources per simulation (only 2 windows: baseline and sources)
...
%% Simulate sources
roiActive = randsample(1:2:numROIs,randi(numROIs/2));
...
if repBoot==1 && numSubs == 50 && level==3
    %%% plot BETAs
    count = 1;
    ...
    saveas(gcf,['figures/randbetaErpSrcN' num2str(numSubs) 'SNR' num2str(SNRlevel(level)) ],'png')
    count = 1;
    figure;set(gcf,'position',[100,100,800,1000])
    for iRoi = 1:2:numROIs
        % need to normalise the signal
    ...
    saveas(gcf,['figures/randbetaErpTempN' num2str(numSubs) 'SNR' num2str(SNRlevel(level)) ],'png')
    count = 1;
    figure;set(gcf,'position',[100,100,800,1000])
    for iRoi = 1:2:numROIs
        % need to normalise the signal
    ...
    saveas(gcf,['figures/randbetaErpRoiN' num2str(numSubs) 'SNR' num2str(SNRlevel(level)) ],'png')
    count = 1;
    figure;set(gcf,'position',[100,100,800,1000])
    for iRoi = 1:2:numROIs
        % need to normalise the signal
    ...
    saveas(gcf,['figures/randbetaErpWholeN' num2str(numSubs) 'SNR' num2str(SNRlevel(level)) ],'png')
...
save('randAreas.mat','aucAve','energyAve','mseAveNorm','aucWhole','energyWhole','mseWholeNorm',...
    'aucROI','energyROI','mseROINorm','aucROIin','energyROIin','mseROINormin')
% AUC includes NaN because some simulations have all areas ON -> should I
% not include these?
...
for ss=1:length(nbSbjToInclude)
    ...
    errorbar(log(SNRlevel),squeeze(mean(aucAve(:,ss,:),'omitnan')),squeeze(std(aucAve(:,ss,:),1,'omitnan')),'LineWidth',2)
    xlabel('log(SNR)')
    ylim([0 1])
    ylabel('AUC')
    title('template')
...
saveas(gcf,['figures' filesep 'randSNRtestNtemplateERP'],'png')

figure;
for ss=1:length(nbSbjToInclude)
    subplot(1,3,1);hold on;
    errorbar(log(SNRlevel),squeeze(mean(aucWhole(:,ss,:),'omitnan')),squeeze(std(aucWhole(:,ss,:),1,'omitnan')),'LineWidth',2)
    xlabel('log(SNR)')
    ylim([0 1])
    ylabel('AUC')
    title('Whole')
...
for ss=1:length(nbSbjToInclude)
    subplot(1,3,1);hold on;
    errorbar(log(SNRlevel),squeeze(mean(aucROI(:,ss,:),'omitnan')),squeeze(std(aucROI(:,ss,:),1,'omitnan')),'LineWidth',2)
    xlabel('log(SNR)')
    ylim([0 1])
    ylabel('AUC')
    title('ROI')
...
legend('N2','N8','N20','N50')
saveas(gcf,['figures' filesep 'randSNRtestNROIERP'],'png')


figure;
for ss=1:length(nbSbjToInclude)
    subplot(1,3,1);hold on;
    errorbar(log(SNRlevel),squeeze(mean(aucROIin(:,ss,:),'omitnan')),squeeze(std(aucROIin(:,ss,:),'omitnan')),'LineWidth',2)
    xlabel('log(SNR)')
    ylim([0 1])
    ylabel('AUC')
    title('ROIin')
...
legend('N2','N8','N20','N50')
saveas(gcf,['figures' filesep 'randSNRtestNROIinERP'],'png')
