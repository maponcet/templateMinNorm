clearvars;close all;
% same prog as simulV1MT but compare noise on forward models = correlated
% or on electrodes = uncorrelated
...
% tested across different levels of noise 
...
%             indFwd_noise=fullFwd{iSub};
            ...
            for iSub=1:numSubs
                ...
                [betaWholeB,lambdaWholeB] = minNormFast(fullFwd{iSub}, squeeze(Yb_avg(iSub,:,:)), nLambdaRidge);
%                 [betaWholeLC, lambdaWholeLC] = minNormFast_lcurve(fullFwd{iSub}, squeeze(Y_avg(iSub,:,:)));
                ...
                [betaROIb, lambdaROIb] = minNormFast([roiFwd{iSub,:}], squeeze(Yb_avg(iSub,:,:)), nLambdaRidge);
%                 [betaROILC, lambdaROILC] = minNormFast_lcurve([roiFwd{iSub,:}], squeeze(Y_avg(iSub,:,:)));
                ...
                regionROIb(iSub,:,:) = cell2mat(arrayfun(@(x) sum(betaROIb(range(x)+1:range(x+1), :)),1:numROIs,'uni',false)');
%                 regionROILC(iSub,:,:) = cell2mat(arrayfun(@(x) sum(betaROILC(range(x)+1:range(x+1), :)),1:numROIs,'uni',false)');
                ...
                regionWholeB(iSub,:,:) = cell2mat(arrayfun(@(x) sum(betaWholeB(idxROIfwd{iSub,x},:)),1:numROIs,'uni',false)');
%                 regionWholeLC(iSub,:,:) = cell2mat(arrayfun(@(x) sum(betaWholeLC(idxROIfwd{iSub,x},:)),1:numROIs,'uni',false)');
                ...
                [betaROIinB(iSub,:,:), lambdaGridMinNormROIinB] = minNormFast(sbjROI, squeeze(Yb_avg(iSub,:,:)), nLambdaRidge);
%                 [betaROIinLC(iSub,:,:), lambdaGridMinNormROIinLC] = minNormFast_lcurve(sbjROI, squeeze(Y_avg(iSub,:,:)));
            ...
            retrieveWholeB = squeeze(mean(regionWholeB,1));
            retrieveROIb = squeeze(mean(regionROIb,1));
            retrieveROIinB = squeeze(mean(betaROIinB,1));
...
%             retrieveROIinLC = squeeze(mean(betaROIinLC,1));
            ...
            simulERP(level).dataCorrelNoise = Yb_avg;
            simulERP(level).noise = SNRlevel(level);
            simulERP(level).beta(1,:,:) = betaAverage;
            simulERP(level).beta(2,:,:) = betaAverageB;
            simulERP(level).beta(3,:,:) = retrieveWhole;
            simulERP(level).beta(4,:,:) = retrieveWholeB;
            simulERP(level).beta(5,:,:) = retrieveROI;
            simulERP(level).beta(6,:,:) = retrieveROIb;
            simulERP(level).beta(7,:,:) = retrieveROIin;
            simulERP(level).beta(8,:,:) = retrieveROIinB;
...
%     end % sbj
    save(['simulOutput/brainNoise/simulV1MTbrainNoiseComp' num2str(repBoot) '.mat'],'simulERP')
