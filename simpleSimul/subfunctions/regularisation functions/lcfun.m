...
eta = phi; rho = phi;
if size(beta,1) > length(s)  % A possible least squares residual.
    LS = true;
    rhoLS2 = beta(end,1)^2;
    beta = beta(1:end-1,:);
else
...
for i = 1:length(lambda)
  ...
  phi(i)  = sum(sum(f.*f1.*abs(xi).^2));
  psi(i)  = sum(sum(cf.*f1.*abs(beta).^2));
  dphi(i) = sum(sum((f1.^2 + f.*f2).*abs(xi).^2));
  dpsi(i) = sum(sum((-f1.^2 + cf.*f2).*abs(beta).^2));
end
