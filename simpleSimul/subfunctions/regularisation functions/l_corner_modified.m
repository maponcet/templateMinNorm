...
% 2021 modified to account for time + use curvature function (discrete) 
% instead of lcfun + don't compute reg_c_cur and no plot
...
if (nargin > 3)
  ...
  if (m>n)  % Take of the least-squares residual.
      %beta = [beta;norm(b0)];
      beta=[beta;repmat(norm(b0),1,size(beta,2))];
  end
end
