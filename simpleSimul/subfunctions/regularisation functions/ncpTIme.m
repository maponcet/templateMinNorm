...
if (strncmp(method,'Tikh',4) | strncmp(method,'tikh',4))
  ...
  beta = reshape(beta(1:p,:),size(beta(1:p,:),1)*size(beta(1:p,:),2),1); 
  ...
  cp = zeros(q,npoints);
  for i=1:npoints
    [dists(i),cp(:,i)] = ncpfun(reg_param(i),s,beta,U(:,1:p));
  end 
  ...
  reg_min = fminbnd('ncpfun',...
    ...
    optimset('Display','off'),s,beta,U(:,1:p)); % Minimizer.
  [dist,cp] = ncpfun(reg_min,s,beta,U(:,1:p));
