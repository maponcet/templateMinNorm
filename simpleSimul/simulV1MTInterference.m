...
%         numSubs = nbSbjToInclude(totSbj);
        ...
        for iSub=1:numSubs
            ...
            % scaling factor can be done uni or bilateral (for both vis and ext sources), does not matter
...
%                 y_stimExt = normFwd{iSub} * sourceDataExt;
                ...
                noise = randn(size(y_stimExt50,1),size(y_stimExt50,2));
...
%     end % sbj
    ...
    save(['simulOutput/interference/simulV1MTinterference' num2str(repBoot) '.mat'],'simulERP')
...
% mseAveNorm = aucAve;
% for repBoot=1:totBoot
%     load(['simulOutput/interference/simulV1MTinterference' num2str(repBoot) '.mat'])
%     for model=1:nbModel
%         for level=1:length(SNRlevel)
