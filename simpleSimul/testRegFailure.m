% test for regularisation failure
% based on compareSystemsRandPair but change nb of sbj & only compute
% template min norm

% simulate and retrieve ERP sources from 2 bilateral sources
% tot ROIs = 4, only 1 ERP window
...
mm(2).dataPath = '/Users/marleneponcet/Documents/data/skeriDATA/forwardEGI128/';
...
% some parameters
SNRlevel = [0.1 200 10000]; % 0.1 means 10 times more noise than signal, 10 means 10 times more signal than noise
    ...
    for totSbj=1:length(nbSbjToInclude)
        ...
        for level=1:length(SNRlevel)
            ...
            for sysNb=1:nbModels
                ...
                [betaOut(:,:,sysNb,level,totSbj,repBoot),lambda(sysNb,level,totSbj,repBoot),...
                    residualNorm(sysNb,level,totSbj,repBoot),solutionNorm(sysNb,level,totSbj,repBoot),...
                    regul(sysNb,level,totSbj,repBoot)] = ...
                    minNormFast_lcurve(mm(sysNb).avMap,squeeze(mean(Y_avg,1)));
...
%% plot
betaAvg = squeeze(mean(betaOut,2));
for sysNb=1:nbModels
    for level=1:length(SNRlevel)
        figure;set(gcf, 'Position', [0 0 1500 600])
        subplot(2,4,1);hold on
        for totSbj=1:length(nbSbjToInclude)
            plot(squeeze(residualNorm(sysNb,level,totSbj,:)))
        end
        legend({'N=2' 'N=8' 'N=20'})
        title('residuals')
        subplot(2,4,2);hold on
        for totSbj=1:length(nbSbjToInclude)
            plot(squeeze(solutionNorm(sysNb,level,totSbj,:)))
        end
        title('solutionNorm')
        subplot(2,4,3);hold on
        for totSbj=1:length(nbSbjToInclude)
            plot(squeeze(lambda(sysNb,level,totSbj,:)))
        end
        title('lambda')
        subplot(2,4,4);hold on
        for totSbj=1:length(nbSbjToInclude)
            plot(squeeze(solutionNorm(sysNb,level,totSbj,:)) ./ squeeze(residualNorm(sysNb,level,totSbj,:)))
        end
        title('S/R')
        subplot(2,4,5);hold on
        imagesc(squeeze(betaAvg(:,sysNb,level,1,:)))
        title('N=2')
        subplot(2,4,6);hold on
        imagesc(squeeze(betaAvg(:,sysNb,level,2,:)));title('N=8')
        subplot(2,4,7);hold on
        imagesc(squeeze(betaAvg(:,sysNb,level,3,:)));title('N=20')
        subplot(2,4,8);hold on
        for totSbj=1:length(nbSbjToInclude)
            plot(squeeze(regul(sysNb,level,totSbj,:)))
        end
        title('bad lambda (extreme+solNorm)')
        saveas(gcf,['figures/regFail2_sys' num2str(sysNb) 'SNR' num2str(level)],'png')
    end
end
