...
for repBoot=1:totBoot
    ...
    srcExtERP(:,46:90) = createSourceERP(length(sourceExt),1,2); % create 1 ERP, repeated for the 2nd source (bilateral)
    ...
    for totSbj=1:length(nbSbjToInclude) 
        ...
        %% LOAD FWD
        fullFwd=cell(1,numSubs);roiFwd=cell(numSubs,numROIs);
        ...
        for level=1:length(SNRlevel)
            ...
            for iSub=1:numSubs
                ...
                % add interfering source to the sourceData
                 sourceDataExt = sourceData; sourceDataExt50= sourceData;
               for ss=1:length(sourceExt)
                    sourceDataExt(idxExtROIfwd{iSub,ss},:) = repmat(srcExtERP(ss,:),length(idxExtROIfwd{iSub,ss}),1);
                    sourceDataExt50(idxExtROIfwd{iSub,ss},:) = repmat(srcExtERP(ss,:)/2,length(idxExtROIfwd{iSub,ss}),1);
                end
                ...
                y_stimExt = fullFwd{iSub} * sourceDataExt;
                y_stimExt50 = fullFwd{iSub} * sourceDataExt50;
                % add noise
...
save('simulOutput/simulV1MTExtSrc.mat','simulERP','-v7.3')
...
saveas(gcf,['figures' filesep 'simulV1MTInterference'],'png')
